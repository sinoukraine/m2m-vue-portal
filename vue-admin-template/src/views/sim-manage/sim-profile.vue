      ...
      <el-card class="box-card m-30">
        <div class="vertical-middle px-5">
          ...
          <div class="card-inline card-panel-right display-flex">
            <div class="big-round" :class="currentStateColor" />
          ...
          <div class="chart-container line-chart-container mt-30">
            <div style="display:flex;justify-content: center;">
              <div style="display:flex">
                <div class="square" :style="'background-color:rgb(182, 162, 222);'" />
                <div class="font-14" style="padding: 0 15px;color:#606268">Data MB</div>
              </div>
              <div style="display:flex">
                <div class="square" :style="'background-color:#ffb880;'" />
                <div class="font-14" style="padding: 0 15px;color:#606268">Average (MB/day)</div>
              </div>
              <div style="display:flex">
                <div class="square" :style="'background-color:#d77980;'" />
     ...
     const imsi = this.$route.params.id
        ...
        $.ajax(settings).done(function(response) {
          if (response.Data.length) {
            const arrLabel = []; const arrData = []
            let totalData = 0
            response.Data.forEach(element => {
              arrLabel.push(element.Date.slice(0, 10))
              const chartData = element.Usage / 1048576
      ...
      fetchSIMListAjax(query).then(responseProfile => {
        if (responseProfile) {
          const objProfile = responseProfile.rows[0]
        ...
        const halfDayAgo = moment(currentTime, 'YYYY-MM-DD HH').add(-12, 'hours').format('YYYY-MM-DD HH')
        const oneDayAgo = moment(currentTime, 'YYYY-MM-DD HH').add(-1, 'days').format('YYYY-MM-DD HH')
        const threeDayAgo = moment(currentTime, 'YYYY-MM-DD HH').add(-3, 'days').format('YYYY-MM-DD HH')

          self.currentState = this.temp.state
          let utcDate = ''
          if (objProfile.DataUpdateTime != null) {
            const activityTime = objProfile.DataUpdateTime
            const utcActivityDate = moment.utc(activityTime).toDate()
            ...
            const simActivityTime = moment(activityTime, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
            ...
            var listQuery_1 = {
              ...
              // $.ajax(settings_1).done(  function (result_1) {
                if (result_1.Data != null && result_1.Data.length) {
                  const dataArr = result_1.Data.split('\r\n')
                  dataArr.pop()
                  dataArr.forEach((element, index) => {
                    const dataJson = element.split(',')
                    const startDate = moment.utc(dataJson[4]).toDate()
                    const endDate = moment.utc(dataJson[5]).toDate()
                ...
                const sortedArr = jsonDataArr.sort(function(a, b) {
        ...
        var settings = {
					'url': `https://test4.m2mdata.co/JT/SIM/GetHlrInfo?imsi=${query.IMSIs}`,
					'method': 'POST',
					'timeout': 0,
					'headers': {
					'token': '00000000-0000-0000-0000-000000000000',
			...
			$.ajax(settings).done(function(resHLR) {
        // removed const responseHLR = await fetch(`https://m2mdata.co/jt/GetGetHlrInfo?imsi=${query.IMSIs}`)
        // let resHLR = await responseHLR.json()

        const mapList = resHLR.Data.dataMap
        const hlrList = resHLR.Data.hlrInfoFields

        const hlrDate1 = hlrList.find(el => el.name == self.simDetailslistRight[0].title).value
				const hlrDate2 = hlrList.find(el => el.name == self.simDetailslistRight[1].title).value

        self.simDetailslistLeft[0].value = hlrList.find(el => el.name == self.simDetailslistLeft[0].title).value
        self.simDetailslistLeft[1].value = mapList.find(el => el.key == self.simDetailslistLeft[1].title)?.value
        self.simDetailslistLeft[2].value = hlrList.find(el => el.name == self.simDetailslistLeft[2].title).value
        self.simDetailslistRight[0].value = hlrDate1 == '00000000000000' ? '' : hlrDate1.slice(0, 4) + '-' + hlrDate1.slice(4, 6) + '-' + hlrDate1.slice(6, 8) + ' ' + hlrDate1.slice(8, 10) + ':' + hlrDate1.slice(10, 12) + ':' + hlrDate1.slice(12, 14)
        self.simDetailslistRight[0].title = 'Data Connect Up Time'
        self.simDetailslistRight[1].value = hlrDate2 == '00000000000000' ? '' : hlrDate2.slice(0, 4) + '-' + hlrDate2.slice(4, 6) + '-' + hlrDate2.slice(6, 8) + ' ' + hlrDate2.slice(8, 10) + ':' + hlrDate2.slice(10, 12) + ':' + hlrDate2.slice(12, 14)
        self.simDetailslistRight[1].title = 'SMS Connect Up Time'
        self.simDetailslistRight[2].value = mapList.find(el => el.key == self.simDetailslistRight[2].title)?.value

        self.mapList = mapList
        const indexToRemove1 = self.mapList.findIndex(el => el.key == 'MSISDN')
        if (indexToRemove1 != -1) {
          self.mapList.splice(indexToRemove1, 1)
        }
        const indexToRemove2 = self.mapList.findIndex(el => el.key == 'APN2')
        if (indexToRemove2 != -1) {
          self.mapList.splice(indexToRemove2, 1)
        }
      })
    },
    refreshState() {
                ...
                if (response.Data.length) {
                  ...
                  if (self.page > 1) {
                    ...
                    if (self.oldHistoryArray.length > 0) {
                      ...
                      const renderArr = self.oldHistoryArray.map(el => ({ ...el, CreateTime: el.CreateTime.slice(0, 19).replace('T', ' '), CenterNumber: el.CenterNumber.toString(), Direction: el.Direction == 2 ? 'Outbound' : 'Inbound', State: el.State == 0 ? 'Error' : el.State == 1 ? 'Sent' : el.State == 2 ? 'Submitted' : el.State == 3 ? 'Delivered' : 'Received' }))
    ...
    showLocation() {
      ...
      // country by coords
       ...
       const queryLBS = {
          ...
          const utcActivityDate = moment.utc(activityTime).toDate()
          const utcDate = utcActivityDate.getDate() + ' ' + self.month_names_short[utcActivityDate.getMonth()] + ' ' + utcActivityDate.getFullYear() + ' ' + ('0' + utcActivityDate.getHours()).slice(-2) + ':' + ('0' + utcActivityDate.getMinutes()).slice(-2) + ':' + ('0' + utcActivityDate.getSeconds()).slice(-2)
