<template>
  <el-container
    v-if="Permission['SIM_LIST']>0"
    class="with-panel-wrapper sim-list-page"
    :class="{'panel-opened': isRightPanelVisible}"
  >
    ...
    <el-dialog title="View Map" :visible.sync="mapFormVisible" width="70%" class="bg-blue">
      <el-row :gutter="16">
          ...
          <div class="map-container">
            ...
            >
              <l-tile-layer
                ...
                layer-type="base"
              />
              <l-marker :lat-lng="markerLatLng" :icon="markerIcon"></l-marker>
              <!--<l-circle 
        ...
        <el-col :xs="24" :sm="12" class="lg-pl-0">
          <el-table :data="locationList" fit :show-header="false" class="location-table">
      ...
      <el-row>
        <div class="card-flex">
          <div class="card-inline card-panel-left"></div>
    ...
    <el-dialog title="Move SIMs to" :visible.sync="moveFormVisible" width="70%" class="bg-white">
      <el-form
        ...
        label-width="70px"
      >
        <!-- @submit.native.prevent="onMoveFormSubmit"-->
          ...
          <el-col :xs="24" :sm="24">
            <el-form-item :label="'Activate to'" prop="ActivateTo">
              <el-select v-model="selectedActivateMode" :placeholder="'Activate to'" class>
                <el-option
                  v-for="item in activateModeOptions"
                  :key="item.Code"
                  :label="item.Name"
                  :value="item.Code"
                />
          ...
          <el-col :xs="24" :sm="24">
            <el-form-item :label="$t('SERVICEPROFILE')" prop="ServiceProfile">
              <el-select v-model="selectedServiceProfile" :placeholder="$t('SERVICEPROFILE')" class>
                <el-option
                  v-for="item in serviceProfileOptions"
                  :key="item.Code"
                  :label="item.Name"
                  :value="item.Code"
                />
    ...
    <el-dialog
      title="Set Service Profile"
      :visible.sync="sspFormVisible"
      width="30%"
      class="bg-white"
    >
      <el-row :gutter="16">
      ...
      <el-main class="no-padding">
          ...
          <div class="display-flex justify-content-between">
            <div class="buttons-row">
              <div v-if="Permission['SIM_LIST']>1">
                <el-button
                  v-if="Permission['SIM_TEST']>1"
                  v-waves
                  class="button-custom"
                  @click="showTestForm"
                >
                  <item :icon="'activation'" />Activate
                </el-button>
                ...
                <el-button
                  v-if="Permission['SIM_SUSPEND_RESUME']>1||Permission['SIM_TERMINATE']>1"
                  v-waves
                  class="button-custom"
                  @click="showStateForm"
                >
                ...
                <el-button
                  v-if="Permission['SIM_CHANGE_CSP']>1"
                  @click="showCSPForm"
                  v-waves
                  class="button-custom"
                >
            ...
            <div class="buttons-row white-space-nowrap">
              <el-button v-waves class="button-custom blue-btn" type="primary" @click="getList">
                <item :icon="'update-white'" />
              </el-button>
              <el-button
                v-waves
                :loading="downloadLoading"
                class="button-custom blue-btn"
                type="primary"
                @click="handleDownload"
              >
                <item :icon="'save-white'" />
              </el-button>
            </div>
          </div>
          <div class="buttons-row">
        ...
        <div class="table-wrapper">
          ...
          >
            ...
            <el-table-column
              :label="$t('IMSI')"
              sortable
              :class-name="getSortClass('IMSI')"
              prop="IMSI"
              align="left"
              width="130px"
            >
              <template slot-scope="{row}">
                <router-link class="link" :to="{ path: `/sim-list/${row.IMSI}` }">{{ row.IMSI }}</router-link>
              </template>
            </el-table-column>
            <el-table-column
              v-if="checkboxICCID"
            ...
            <el-table-column
              ...
              sortable
              :label="$t('MSISDN')"
              :class-name="getSortClass('MSISDN')"
              prop="MSISDN"
              align="left"
              width="140px"
            >
            ...
            <el-table-column
              v-if="checkboxServiceProfile"
              sortable
              :label="$t('SERVICEPROFILE')"
              :class-name="getSortClass('ServiceProfileCode')"
              prop="ServiceProfileCode"
              align="left"
              min-width="130px"
            >
              <template slot-scope="{row}">
            ...
            <el-table-column
              v-if="checkboxOrganize"
              sortable
              :class-name="getSortClass('OrganizeName')"
              prop="OrganizeName"
              :label="$t('CUSTOMER')"
              align="left"
              min-width="100px"
            ...
            <el-table-column
              v-if="checkboxDataSession"
              sortable
              :class-name="getSortClass('SessionDay')"
              prop="SessionDay"
              :label="'Session'"
              align="left"
              min-width="70px"
            ...
            <el-table-column
              v-if="checkboxDataUsage"
              :label="'Data'"
              sortable
              :class-name="getSortClass('DataDay')"
              prop="DataDay"
              align="left"
              min-width="70px"
            >
            ...
            <el-table-column
              v-if="checkboxSMSUsage"
              :label="'SMS'"
              sortable
              :class-name="getSortClass('SMSMtDay')"
              prop="SMSMtDay"
              align="left"
              min-width="70px"
            >
            ...
            <el-table-column
              v-if="checkboxPayPlan"
              :label="$t('PAYPLAN')"
              sortable
              :class-name="getSortClass('PayPlanCode')"
              prop="PayPlanCode"
              align="left"
              min-width="100px"
            >
            ...
            <el-table-column
              v-if="checkboxIPAddress"
              :label="$t('IPADDRESS')"
              sortable
              :class-name="getSortClass('IPAddress')"
              prop="IPAddress"
              align="left"
              min-width="100px"
            >
            ...
            <el-table-column
              v-if="checkboxCustomField1"
              :label="$t('CUSTOMFIELD1')"
              sortable
              :class-name="getSortClass('CustomField1')"
              prop="SuctomField1"
              align="left"
              min-width="120px"
            >
            ...
            <el-table-column
              v-if="checkboxCustomField2"
              :label="$t('CUSTOMFIELD2')"
              sortable
              :class-name="getSortClass('CustomField2')"
              prop="SuctomField2"
              align="left"
              min-width="120px"
            >
            ...
            <el-table-column
              v-if="checkboxCustomField3"
              :label="$t('CUSTOMFIELD3')"
              sortable
              :class-name="getSortClass('CustomField3')"
              prop="SuctomField3"
              align="left"
              min-width="120px"
            >
            ...
            <el-table-column
              v-if="checkboxCustomField4"
              :label="$t('CUSTOMFIELD4')"
              sortable
              :class-name="getSortClass('CustomField4')"
              prop="SuctomField4"
              align="left"
              min-width="120px"
            >
            ...
            <el-table-column
              v-if="checkboxCustomField5"
              :label="$t('CUSTOMFIELD5')"
              sortable
              :class-name="getSortClass('CustomField5')"
              prop="SuctomField5"
              align="left"
              min-width="120px"
            >
            ...
            <el-table-column
              :label="'Update time (UTC)'"
              sortable
              :class-name="getSortClass('DataUpdateTime')"
              prop="DataUpdateTime"
              align="left"
              min-width="90px"
            >
            ...
            <el-table-column
              v-if="Permission['SIM_LIST']>1&&Permission['SIM_EDIT']>1"
              label="Actions"
              align="center"
              width="100"
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="isDialogFormVisible">
          <el-form
            ...
            label-position="top"
            label-width="70px"
            @submit.native.prevent="onEditFormSubmit"
          >
            <input type="submit" class="display-none" />

            <el-row v-if="dialogStatus !== 'create'" :gutter="16">
              <el-col :xs="24" :sm="24" class="text-right"></el-col>
            </el-row>
    ...
    <el-aside class="panel panel-right">
      <div
        class="panel-open display-flex justify-content-between align-items-center"
        ...
        <el-row :gutter="16">
          <el-col :xs="100">
            <label
              v-waves
              :for="filterSubmitId"
              class="el-button el-button--primary width-100 blue-btn"
            >
      ...
      <div class="panel-container">
        <el-row :gutter="0">
          <el-col :xs="100">
            <div
              class="panel-hide padding-horizontal-x2 display-flex justify-content-between align-items-center"
              @click="isRightPanelVisible = !isRightPanelVisible"
            >
            ...
            <el-row :gutter="16" class="p-7">
              <el-col :xs="100" class="px-0">
                <el-form-item label="Query" prop="fromimsi" class>
                  <el-input
                    v-model="listQuery.q"
                    placeholder="Enter 5 symbols minimum"
                    class="filter-item"
                  />
                </el-form-item>
              </el-col>
              ...
              <el-col :xs="100" class="px-0">
                <el-form-item label="From IMSI" prop="fromimsi" class>
                  <el-input
                    v-model="listQuery.FromIMSI"
                    placeholder="From IMSI"
                    class="filter-item"
                  />
                </el-form-item>
              </el-col>
              ...
              <el-col :xs="100" class="px-0">
                <el-form-item label="From ICCID" prop="fromiccid" class>
                  <el-input
                    v-model="listQuery.FromICCID"
                    placeholder="From ICCID"
                    class="filter-item"
                  />
                </el-form-item>
              </el-col>
            ...
            <el-row :gutter="16">
              <el-col :xs="100">
                <el-form-item :label="$t('RAG')" prop="title" class>
                  <el-checkbox v-model="checkboxSearchRAGr">
                    <span class="rag bg-color-red"></span>
                  </el-checkbox>
                  <el-checkbox v-model="checkboxSearchRAGy">
                    <span class="rag bg-color-yellow"></span>
                  </el-checkbox>
                  <el-checkbox v-model="checkboxSearchRAGg">
                    <span class="rag bg-color-green"></span>
                  </el-checkbox>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <div class="padding-horizontal-x2 pt-10">
            <el-form-item :label="$t('STATE')" prop="title" class>
              <el-row :gutter="16" style>
                ...
                <el-col :span="12">
                  <el-checkbox
                    v-model="checkboxSearchStatusTestProductive"
                  >{{ $t('TEST_PRODUCTIVE') }}</el-checkbox>
                </el-col>
              </el-row>
            </el-form-item>
          </div>
          ...
          <div class="padding-horizontal-x2 pt-10">
            <el-form-item :label="$t('CSP')" prop="title" class>
              <el-row :gutter="16" style>
                ...
                <el-col :span="12">
                  <el-checkbox
                    v-model="checkboxSearchCSPM2MDataVodafoneAustralia"
                  >Custom Profile Tier 1</el-checkbox>
                </el-col>
              </el-row>
            </el-form-item>
          </div>
  ...
  <div v-else class="no-data-info">
    <div class="py-20">Permission denied</div>
  </div>
</template>
...
import {
  setTestAjax,
  fetchSIMListAjax,
  fetchCustomersListAjax,
  setActivateStateAjax,
  setSuspendStateAjax,
  setResumeStateAjax,
  setTerminateStateAjax,
  setServiceProfileOptionsAjax,
  fetchServiceProfileOptionsAjax,
  createSIMAjax,
  updateSIMAjax,
  moveSIMsAjax
} from "@/api/user";
    ...
    return {
      ...
      rulesMove: {
        Remark: [
          {
            required: false,
            message: "Remark cannot be longer than 100 characters",
            max: 100
          }
        ],
        Number: [
          {
            required: false,
            message: "Number cannot be longer than 50 characters",
            max: 50
          }
        ],
        FromIMSI: [
          {
            required: true,
            message: "From IMSI should be 15 characters long",
            min: 15,
            max: 15
          }
        ],
        ToIMSI: [
          {
            required: true,
            message: "To IMSI should be 15 characters long",
            min: 15,
            max: 15
          }
        ],
        FromICCID: [
          {
            required: true,
            message: "From ICCID should be 19 characters long",
            min: 19,
            max: 19
          }
        ],
        ToICCID: [
          {
            required: true,
            message: "TO ICCID should be 19 characters long",
            min: 19,
            max: 19
          }
        ]
      ...
      rules: {
        ...
        ParentCode: [
          { required: true, message: "Parent is required", trigger: "change" }
        ],
        ServiceProfileCode: [
          { required: true, message: "Parent is required", trigger: "change" }
        ],
  ...
  methods: {
    ...
    showLocation(data) {
      ...
      $.ajax(settingsLBS).done(function(result) {
        ...
        let utcDate =
          utcActivityDate.getDate() +
          " " +
          self.month_names_short[utcActivityDate.getMonth()] +
          " " +
          utcActivityDate.getFullYear() +
          " " +
          ("0" + utcActivityDate.getHours()).slice(-2) +
          ":" +
          ("0" + utcActivityDate.getMinutes()).slice(-2) +
          ":" +
          ("0" + utcActivityDate.getSeconds()).slice(-2);
        ...
        self.locationList[6].value = result.Data.LbsRadio
          ? result.Data.LbsRadio + " " + result.Data.LbsRange
          : "";
    ...
    async saveTest() {
      ...
      if (this.selectedActivateMode == "Productive") {
        for (let i = 0; i < this.multipleSIMSelection.length; i++) {
          if (
            this.multipleSIMSelection[i].State == "OnStock" ||
            this.multipleSIMSelection[i].State == "TestProductive" ||
            this.multipleSIMSelection[i].State == "TestEnd"
          ) {
        ...
        if (!checkSIM || this.multipleSIMSelection.length == 0) {
          this.$alert(
            "Only support the sim state in OnStock / TestProductive / TestEnd",
            "M2M Data Message",
            { type: "message" }
          );
        } else {
          ...
          setActivateStateAjax(query)
            .then(r => {
              if (r.MajorCode == "000") {
                this.testFormVisible = false;
                this.$alert("Activated", "M2M Data Message", {
                  type: "message"
                });
                this.getList();
              } else {
                this.$alert(
                  "Error, Current sim state not available. Refresh page and try again.",
                  "M2M Data Message",
                  { type: "message" }
                );
              }
            ...
            .catch(e => {
              this.$alert("SIM/s can not be activated", "M2M Data Message", {
                type: "message"
              });
            });
        }
        ...
        if (!checkSIM || this.multipleSIMSelection.length == 0) {
          this.$alert(
            "Only support the sim state in OnStock",
            "M2M Data Message",
            { type: "message" }
          );
        } else {
          ...
          setTestAjax(query)
            .then(r => {
              if (r.MajorCode == "000") {
                this.testFormVisible = false;
                this.$alert(
                  "Set TestProductive was submitted",
                  "M2M Data Message",
                  { type: "message" }
                );
                this.getList();
            ...
            .catch(e => {
              this.$alert(
                "SIM/s can not be set on TestProductive",
                "M2M Data Message",
                { type: "message" }
              );
            });
    ...
    async saveState() {
      ...
      for (let i = 0; i < this.multipleSIMSelection.length; i++) {
        switch (this.selectedState) {
          case "0":
            if (
              this.multipleSIMSelection[i].State == "Productive" ||
              this.multipleSIMSelection[i].State == "TestProductive"
            ) {
              arr.push(this.multipleSIMSelection[i].IMSI);
            } else {
          ...
          case "2":
            if (
              this.multipleSIMSelection[i].State == "Productive" ||
              this.multipleSIMSelection[i].State == "Suspended"
            ) {
              //removed  || this.multipleSIMSelection[i].State == 'TestProductive'
              arr.push(this.multipleSIMSelection[i].IMSI);
            } else {
              checkSIM = false;
            }
      ...
      if (!checkSIM || this.multipleSIMSelection.length == 0) {
        switch (this.selectedState) {
          case "0":
            this.$alert(
              "Only support the sim state in Productive / TestProductive",
              "M2M Data Message",
              { type: "message" }
            );
            break;
          case "1":
            this.$alert(
              "Only support the sim state in Suspended",
              "M2M Data Message",
              { type: "message" }
            );
          ...
          case "2":
            this.$alert(
              "Only support the sim state in Productive / Suspended",
              "M2M Data Message",
              { type: "message" }
            ); //removed  / TestProductive
        ...
        switch (this.selectedState) {
          case "0":
            setSuspendStateAjax(query)
              .then(r => {
                if (r.MajorCode == "000") {
                  this.$alert("Set suspend successfuly", "M2M Data Message", {
                    type: "message"
                  });
                  this.stateFormVisible = false;
                  this.getList();
              ...
              .catch(e => {
                this.$alert("SIM/s can not be suspended", "M2M Data Message", {
                  type: "message"
                });
              });
            break;
          case "1":
            setResumeStateAjax(query)
              .then(r => {
                if (r.MajorCode == "000") {
                  this.$alert("Set resume successfuly", "M2M Data Message", {
                    type: "message"
                  });
                  this.stateFormVisible = false;
                  this.getList();
              ...
              .catch(e => {
                this.$alert("SIM/s can not resume", "M2M Data Message", {
                  type: "message"
                });
              });
            break;
          case "2":
            setTerminateStateAjax(query)
              .then(r => {
                if (r.MajorCode == "000") {
                  this.$alert("SIM/s terminated", "M2M Data Message", {
                    type: "message"
                  });
                  this.stateFormVisible = false;
                  this.getList();
              ...
              .catch(e => {
                this.$alert("SIM/s can not be terminated", "M2M Data Message", {
                  type: "message"
                });
              });
            break;
    ...
    async saveServiceProfile() {
      ...
      if (!checkSIM) {
        this.$alert(
          "There are no SIM to Set Service Profile",
          "M2M Data Message",
          { type: "message" }
        );
        ...
        setServiceProfileOptionsAjax(query)
          .then(r => {
            if (r.MajorCode == "000") {
              this.$alert(
                "Service Profile was set successfully",
                "M2M Data Message",
                { type: "message" }
              );
              this.sspFormVisible = false;
              ...
              this.$alert(
                "Error, Current csp state not available. Refresh page and try again.",
                "M2M Data Message",
                { type: "message" }
              );
            }
          ...
          .catch(e => {
            this.$alert("Something wrong...", "M2M Data Message", {
              type: "message"
            });
          });
      }
      ...
      this.$refs["dataFormMove"].validate(async valid => {
        ...
        if (this.tempMove.searchedOrganize?.Code != undefined) {
          ...
          if (!checkSIM && this.tempMove.sendWay == "0") {
            this.$alert("There are no SIM to move", "M2M Data Message", {
              type: "message"
            });
          } else {
            ...
            moveSIMsAjax(query)
              .then(r => {
                if (r.MajorCode == "000") {
                  ...
                  this.$alert("SIMs was not moved", "M2M Data Message", {
                    type: "message"
                  });
                }
              })
          ...
          this.$alert("Select Organize to move SIMs", "M2M Data Message", {
            type: "message"
          });
        }
      });
    ...
    async getList() {
      ...
      let halfDayAgo = moment(currentTime, "YYYY-MM-DD HH")
        .add(-12, "hours")
        .format("YYYY-MM-DD HH");
      let oneDayAgo = moment(currentTime, "YYYY-MM-DD HH")
        .add(-1, "days")
        .format("YYYY-MM-DD HH");
      let threeDayAgo = moment(currentTime, "YYYY-MM-DD HH")
        .add(-3, "days")
        .format("YYYY-MM-DD HH");
        ...
        if (!response.MajorCode) {
          ...
          response.rows.forEach(async element_1 => {
            ...
            arr.push({
              ...
              DataUsage: element_1.DataDay
                ? (element_1.DataDay / 1048576).toFixed(3).toString()
                : 0,
    ...
    handleFilter() {
      ...
      this.checkboxSearchStatusOnStock
        ? this.listQuery.Status.push("OnStock")
        : null;
      this.checkboxSearchStatusSuspended
        ? this.listQuery.Status.push("Suspended")
        : null;
      this.checkboxSearchStatusProductive
        ? this.listQuery.Status.push("Productive")
        : null;
      this.checkboxSearchStatusTest ? this.listQuery.Status.push("Test") : null;
      this.checkboxSearchStatusTestProductive
        ? this.listQuery.Status.push("TestProductive")
        : null;
      this.checkboxSearchStatusTestEnd
        ? this.listQuery.Status.push("TestEnd")
        : null;
      this.checkboxSearchStatusActivated
        ? this.listQuery.Status.push("Activated")
        : null;
      this.checkboxSearchStatusOrdered
        ? this.listQuery.Status.push("Ordered")
        : null;
      this.checkboxSearchStatusAutoSuspended
        ? this.listQuery.Status.push("AutoSuspended")
        : null;
      this.checkboxSearchStatusReserved
        ? this.listQuery.Status.push("Reserved")
        : null;
      this.checkboxSearchStatusDeleted
        ? this.listQuery.Status.push("Deleted")
        : null;
      this.checkboxSearchStatusPaused
        ? this.listQuery.Status.push("Paused")
        : null;
      this.checkboxSearchStatusLost ? this.listQuery.Status.push("Lost") : null;
      this.checkboxSearchStatusEuiccControl
        ? this.listQuery.Status.push("EuiccControl")
        : null;
      this.checkboxSearchStatusDeleting
        ? this.listQuery.Status.push("Deleting")
        : null;
      this.checkboxSearchStatusPassive
        ? this.listQuery.Status.push("Passive")
        : null;

      this.checkboxSearchCSPM2MData
        ? this.listQuery.ServiceprofileCodes.push("M2M Data")
        : null;
      this.checkboxSearchCSPM2MDataEmergency
        ? this.listQuery.ServiceprofileCodes.push(
            "M2M Data Emergency CSP Z1- 9"
          )
        : null;
      this.checkboxSearchCSPM2MDataNormal
        ? this.listQuery.ServiceprofileCodes.push("M2M Data Normal CSP Z1 &2")
        : null;
      this.checkboxSearchCSPM2MDataHigh
        ? this.listQuery.ServiceprofileCodes.push("M2M Data High CSP Z1-3")
        : null;
      this.checkboxSearchCSPM2MDataVodafoneAustralia
        ? this.listQuery.ServiceprofileCodes.push(
            "M2M Data Custom Network Profile Australia"
          )
        : null;
      this.checkboxSearchCSPM2MdataTelstraOnly
        ? this.listQuery.ServiceprofileCodes.push("M2M Data Telstra Only")
        : null;
    ...
    sortChange(data) {
      ...
      this.listQuery.Order =
        this.listQuery.Order === "ASC"
          ...
          ? "ASC"
          : "ASC"; //order === 'ascending' ? 'ASC' : (order === 'descending') ? 'DESC' : 'ASC'
    ...
    async forceReconnect() {
      ...
      $.ajax(settingsLBS).done(function(result) {
        ...
        if (result.MajorCode == "000") {
          ...
          //self.showLocation()
          self.$alert("Sim connection refreshed", "M2M Data Message", {
            type: "message"
          });
        } else {
          self.$alert("Sim connection was not refreshed", "M2M Data Message", {
            type: "message"
          });
        }
      });
