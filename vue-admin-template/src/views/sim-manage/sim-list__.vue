...
<el-container class="with-panel-wrapper " :class="{'panel-opened': isRightPanelVisible}">
  ...
  <el-dialog title="Set Service Profile" :visible.sync="cspFormVisible" width="70%" >
      <el-row :gutter="16">
        <el-col :xs="24" :sm="24" class="lg-pr-0">
          <div class="state-container">
            <el-select v-model="selectedCSP" :placeholder="$t('CSP')" class="">
      ...
      <el-row>
        <div class="card-flex">
          ...
          <div class="card-inline card-panel-right">
            <el-button type="info" class="blue-btn mt-25" @click="setCSP">Save</el-button>
          </div>
        </div>
      </el-row>
  </el-dialog>
  ...
  <el-dialog title="Set State" :visible.sync="stateFormVisible" width="70%" >
      <el-row :gutter="16">
        <el-col :xs="24" :sm="6" class="lg-pr-0 align-right">
            <el-button type="primary green-btn w-100" >
              Activate
            </el-button>
        </el-col>
        <el-col :xs="24" :sm="6" class="lg-pr-0 align-right">
            <el-button type="primary orange-btn w-100" >
              Suspend
            </el-button>
        </el-col>
        <el-col :xs="24" :sm="6" class="lg-pr-0 align-right">
            <el-button type="primary blue-btn w-100" >
              Resume
            </el-button>
        </el-col>
        <el-col :xs="24" :sm="6" class="lg-pr-0 align-right">
            <el-button type="primary dark-btn w-100" >
              Terminate
            </el-button>
        </el-col>
      </el-row>
        ...
        <el-main  class="no-padding">
            <div class="filter-container ">
                ...
                <div class="buttons-row">
                    ...
                    <el-button v-waves class="button-custom" @click="showCSPForm"><item :icon="'csp-blue'"/> {{ $t('CSP') }}</el-button>
            ...
            >
              ...
              <el-table-column
                ...
                <el-table-column :label="$t('IMSI')"   :class-name="getSortClass('imsi')" align="left" width="140px">
                ...
                <el-table-column :label="$t('CSP')" :class-name="getSortClass('ServiceProfileCode')" align="left" min-width="180px">
                  <template slot-scope="{row}">
                    <span>{{ row.ServiceProfileCode }}</span>
                  </template>
                </el-table-column>
                <el-table-column :label="$t('ORGANIZE')"  :class-name="getSortClass('Organize')" align="left" min-width="100px">
                  <template slot-scope="{row}">
                    <span>{{ row.OrganizeName }}</span>
                  </template>
                </el-table-column>
                <el-table-column :label="$t('PAYPLAN')"  :class-name="getSortClass('PayPlanCode')" align="left" min-width="100px">
                  <template slot-scope="{row}">
                    <span>{{ row.PayPlanCode }}</span>
                  </template>
                </el-table-column>
                <el-table-column :label="$t('IPADDRESS')"  align="left" min-width="100px">
                  <template slot-scope="{row}">
                ...
                <el-table-column :label="$t('UPDATETIME')"  align="left" min-width="120px">
                  <template slot-scope="{row}">
                    <span>{{ row.DataUpdateTime }}</span>
                  </template>
                </el-table-column>
...
import { fetchSIMList, createSIM, updateSIM, deleteCustomer, fetchServiceProfileList, changeOrgState } from "@/api/user";
...
export default {
  ...
  methods: {    
    ...
    acctivateSIM(){
      ...
      console.log(this.multipleSelection[i].State)
        if(this.multipleSelection[i].State == 'OnStock' || this.multipleSelection[i].State == 'TestProductive'){
      ...
      if(checkSIM){
        ...
        this.$alert('There are no SIM to Activate', 'M2M Data Message', {type: 'message'}) 
      }
    },
    ...
    async getServiceProfileOptions(){
      ...
      this.selectedCSP = this.serviceProfileOptions[0]/*
